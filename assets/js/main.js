	...
	$(function() {
		...
		// Banner2.
			...
			if ($banner2.length > 0) {
				...
				// IE fix.
					if (skel.vars.IEVersion < 12) {
						...
						$window.on('resize', function() {
							...
							$banner2.css('height', 'auto');

							window.setTimeout(function() {

								if (bh < wh)
									$banner2.css('height', wh + 'px');
				...
				// Video check.
					var video = $banner2.data('video');

					if (video)
						$window.on('load.banner2', function() {

							// Disable banner2 load event (so it doesn't fire again).
